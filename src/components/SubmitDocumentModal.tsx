...
import { uploadFile, validateFile, formatFileSize, ALLOWED_FILE_TYPES } from '../services/cloudinary';

interface Props {
  open: boolean;
  onClose: () => void;
  onSuccess: (retId: string) => void;
  user: AppUser;
}
...
export const SubmitDocumentModal: React.FC<Props> = ({ open, onClose, onSuccess, user }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.department.trim()) { setError('Department / office is required.'); return; }
    setError('');
    setUploading(true);
    ...
    try {
      ...
      setForm({ title: '', type: '', department: '', remarks: '' });
      setFile(null);
      setUploadProgress(0);
      onSuccess(retId);
    } catch (e: any) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth PaperProps={{ sx: { bgcolor: '#1a2e45', backgroundImage: 'none' } }}>
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', pb: 1.5, borderBottom: '1px solid rgba(201,149,42,0.2)' }}>
        <Box>
          <Typography sx={{ fontWeight: 700, color: '#fff', fontSize: '1rem' }}>Submit RET Document</Typography>
          <Typography sx={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: '0.62rem', color: '#8fa3b8', mt: 0.3, letterSpacing: '1px' }}>
            Process 1.0 — Submit Document
          </Typography>
        </Box>
        <IconButton onClick={handleClose} disabled={uploading} size="small" sx={{ color: '#8fa3b8' }}>
      ...
      <DialogContent sx={{ pt: 3 }}>
        ...
        {uploading && (
          <Box sx={{ mb: 2 }}>
            <Typography sx={{ fontSize: '0.72rem', color: '#8fa3b8', mb: 0.5 }}>
              {uploadProgress < 70 ? 'Uploading file to Cloudinary...' : uploadProgress < 90 ? 'Saving to database...' : 'Finalizing...'}
            </Typography>
        ...
        <Grid container spacing={2.5}>
          ...
          <Grid item xs={12}>
            <TextField
              ...
              disabled={uploading}
              placeholder="Optional — add any notes for the VP reviewer..."
              InputLabelProps={{ sx: { fontSize: '0.8rem' } }}
              inputProps={{ style: { fontSize: '0.85rem' } }}
            />
          </Grid>
          <Grid item xs={12}>
            <Typography sx={{ fontSize: '0.72rem', fontWeight: 600, letterSpacing: '1.5px', color: '#8fa3b8', textTransform: 'uppercase', mb: 1 }}>
              Attach Document
            </Typography>
            {file ? (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 2, border: '1px solid rgba(201,149,42,0.3)', borderRadius: 1.5, bgcolor: 'rgba(201,149,42,0.05)' }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography sx={{ fontSize: '0.7rem', color: '#8fa3b8', fontFamily: "'IBM Plex Mono',monospace" }}>{formatFileSize(file.size)}</Typography>
                </Box>
              ...
              <Box
                ...
                onDrop={handleDrop}
                sx={{
                  border: `2px dashed ${dragging ? '#c9952a' : 'rgba(201,149,42,0.25)'}`,
                  borderRadius: 1.5,
                  p: 4,
                  textAlign: 'center',
                  cursor: uploading ? 'not-allowed' : 'pointer',
                  bgcolor: dragging ? 'rgba(201,149,42,0.05)' : 'transparent',
                  transition: 'all 0.15s',
                  '&:hover': { borderColor: '#c9952a', bgcolor: 'rgba(201,149,42,0.03)' },
                }}
              >
                <CloudUploadIcon sx={{ fontSize: 36, color: '#8fa3b8', mb: 1 }} />
                ...
                <Typography sx={{ fontSize: '0.7rem', color: '#8fa3b8', mt: 0.5 }}>
                  {ALLOWED_FILE_TYPES.join(', ')} &mdash; max 10 MB
                </Typography>
              </Box>
            )}
            <input ref={fileRef} type="file" hidden accept={ALLOWED_FILE_TYPES.join(',')} onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} />
          </Grid>
        </Grid>
      </DialogContent>
      ...
      <DialogActions sx={{ px: 3, pb: 3, pt: 2, borderTop: '1px solid rgba(201,149,42,0.2)', gap: 1 }}>
        ...
        <Button onClick={handleSubmit} disabled={uploading} variant="contained" sx={{ fontSize: '0.72rem', minWidth: 160 }}>
